...
class Compl:
    def __init__(self, x, y): # ініціалізація конструктора
        if x == 0 and y == 0: # перевірка значень
            # кидає виняток про помилкове значення
            raise ValueError("Undefined complex number phase")
        # self.x = x  присвоєння 'х' поданого в аргументі в 'х' об`єкта класу
        self.x = x
        self.y = y
...
class Fraction:
    def __init__(self, m, n): # ініціалізація конструктора
        ...
        # self.m = m  присвоєння 'm' поданого в аргументі в 'm' об`єкта класу   
        self.m = m
        self.n = n
...
try:  # `спроба` виконати код в блоці
    ...
    cc = ac * bc # ініціалізація об`єкта cc з добутком ac i bc
    cc.print() # друк цього об`єктa
    
    print("Fraction number multiplication(operator):")
    cf = af * bf # ініціалізація об`єкта cf з добутком af i bf
    ...
    # Сортування через lambda-функцію, сортування за зростанням 
